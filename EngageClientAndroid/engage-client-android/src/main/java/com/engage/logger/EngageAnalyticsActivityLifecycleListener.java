 ...
 */
public class EngageAnalyticsActivityLifecycleListener {
    protected static Logger logger = Logger.getLogger(LogPersister.INTERNAL_PREFIX + EngageAnalyticsActivityLifecycleListener.class.getSimpleName());
    ...
    protected static EngageAnalyticsActivityLifecycleListener instance;
    ...
    public static EngageAnalyticsActivityLifecycleListener getInstance() {
        if (instance == null) {
            instance = new EngageAnalyticsActivityLifecycleListener();
        }
        return instance;
    }

    private EngageAnalyticsActivityLifecycleListener() {
        isPaused = true;
        handler = new Handler();
    ...
    protected void logAppForeground() {
        if (appUseStartTimestamp == null) {
            ...
            try {
                metadata.put(EngageAnalytics.DEVICEID, EngageUtils.getDeviceId());
                metadata.put(EngageAnalytics.USERID, EngageAnalytics.getUserId());
                metadata.put(EngageAnalytics.CATEGORY, APP_SESSION_CATEGORY);
                metadata.put(EngageAnalytics.TIMESTAMP_KEY, appUseStartTimestamp);
                metadata.put(EngageAnalytics.APP_SESSION_ID_KEY, appSessionID);
            } catch (JSONException e) {
                // should not happen
                logger.debug("JSONException encountered logging app session: " + e.getMessage());
            }
            EngageAnalytics.log(metadata);
        }
    }
    ...
    private void logAppSession(boolean isCrash) {
        ...
        try {
            metadata.put(EngageAnalytics.DEVICEID, EngageUtils.getDeviceId());
            metadata.put(EngageAnalytics.USERID, EngageAnalytics.getUserId());
            metadata.put(EngageAnalytics.TIMESTAMP_KEY, timestamp);
            metadata.put(EngageAnalytics.CATEGORY, APP_SESSION_CATEGORY);
