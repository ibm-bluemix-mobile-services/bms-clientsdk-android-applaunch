...
public class EngageAnalytics {
    ...
    static public void init(Application app,String userId, String applicationName, String clientApiKey, boolean hasUserContext, Analytics.DeviceEvent... contexts) {
        ...
        Analytics.setAnalyticsDelegate(new EngageAnalyticsDelegate());

        EngageAnalytics.clientApiKey = clientApiKey;
        ...
        EngageAnalytics.hasUserContext = hasUserContext;
     ...
     * accumulates in the log buffer from the use of {@link EngageAnalytics} with capture
     * (see {@link EngageAnalytics#enable()}) turned on.
     *
     */
    ...
    private static void setUserIdentity(final String user, boolean isInitialCtx){
        ...
        if (!isInitialCtx && !EngageAnalytics.hasUserContext) {
            // log it to file:
        ...
        try {
            ...
            metadata.put(APP_SESSION_ID_KEY, EngageAnalyticsActivityLifecycleListener.getAppSessionID());
