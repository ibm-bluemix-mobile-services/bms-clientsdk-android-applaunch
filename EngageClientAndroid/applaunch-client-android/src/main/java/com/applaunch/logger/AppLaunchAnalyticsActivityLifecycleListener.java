 ...
 */
public class AppLaunchAnalyticsActivityLifecycleListener {
    protected static Logger logger = Logger.getLogger(LogPersister.INTERNAL_PREFIX + AppLaunchAnalyticsActivityLifecycleListener.class.getSimpleName());
    ...
    protected static AppLaunchAnalyticsActivityLifecycleListener instance;
    ...
    public static AppLaunchAnalyticsActivityLifecycleListener getInstance() {
        if (instance == null) {
            instance = new AppLaunchAnalyticsActivityLifecycleListener();
        }
        return instance;
    }

    private AppLaunchAnalyticsActivityLifecycleListener() {
        isPaused = true;
        handler = new Handler();
    ...
    protected void logAppForeground() {
        if (appUseStartTimestamp == null) {
            ...
            try {
                metadata.put(AppLaunchAnalytics.DEVICEID, AppLaunchUtils.getDeviceId());
                metadata.put(AppLaunchAnalytics.USERID, AppLaunchAnalytics.getUserId());
                metadata.put(AppLaunchAnalytics.CATEGORY, APP_SESSION_CATEGORY);
                metadata.put(AppLaunchAnalytics.TIMESTAMP_KEY, appUseStartTimestamp);
                metadata.put(AppLaunchAnalytics.APP_SESSION_ID_KEY, appSessionID);
    ...
    private void logAppSession(boolean isCrash) {
        ...
        try {
            metadata.put(AppLaunchAnalytics.DEVICEID, AppLaunchUtils.getDeviceId());
            metadata.put(AppLaunchAnalytics.USERID, AppLaunchAnalytics.getUserId());
            metadata.put(AppLaunchAnalytics.TIMESTAMP_KEY, timestamp);
            metadata.put(AppLaunchAnalytics.CATEGORY, APP_SESSION_CATEGORY);
