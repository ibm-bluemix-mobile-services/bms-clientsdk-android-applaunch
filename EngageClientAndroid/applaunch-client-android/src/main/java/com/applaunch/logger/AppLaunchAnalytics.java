...
public class AppLaunchAnalytics {
     ...
     * @param userId Unique user id provided by the developer
     ...
     */
    static public void init(Application app,String userId, String applicationName, String clientApiKey, boolean hasUserContext, Analytics.DeviceEvent... contexts) {
        Context context = app.getApplicationContext();
        AppLaunchAnalytics.userId = userId;
        ...
        Analytics.setAnalyticsDelegate(new AppLaunchAnalyticsDelegate());

        AppLaunchAnalytics.clientApiKey = clientApiKey;

        if(contexts != null){
            for(Analytics.DeviceEvent event : contexts){
                switch(event){
                    case LIFECYCLE:
                        EngageActivityLifeCycleCallbackListener.init(app);
                        break;
                    case NETWORK:
        ...
        AppLaunchAnalytics.hasUserContext = hasUserContext;
        appName = applicationName;


        //Intercept requests to add device metadata header
      //  BaseRequest.registerInterceptor(new MetadataHeaderInterceptor(context));
     ...
     *
     * @deprecated  As of release 1.1.0, replaced by {@link #init(Application,String, String, String, boolean, Analytics.DeviceEvent...)}}
    ...
    static public void init(Application app, String applicationName, String clientApiKey, Analytics.DeviceEvent... contexts) {
        init(app,null, applicationName, clientApiKey, false, contexts);
    }
     ...
     * accumulates in the log buffer from the use of {@link AppLaunchAnalytics} with capture
     * (see {@link AppLaunchAnalytics#enable()}) turned on.
     *
     */
    ...
    /**
     * See {@link AppLaunchAnalytics#send()}
     *
    ...
    private static void setUserIdentity(final String user, boolean isInitialCtx){
        ...
        if (!isInitialCtx && !AppLaunchAnalytics.hasUserContext) {
            // log it to file:
        ...
        try {
            ...
            metadata.put(APP_SESSION_ID_KEY, AppLaunchAnalyticsActivityLifecycleListener.getAppSessionID());
    ...
    private static class EngageActivityLifeCycleCallbackListener implements Application.ActivityLifecycleCallbacks {
        ...
        public static void init(Application app) {
		 ...
		 AppLaunchAnalyticsActivityLifecycleListener.getInstance().onResume();
            }
        }
        ...
        public void onActivityPaused(Activity activity) {
            AppLaunchAnalyticsActivityLifecycleListener.getInstance().onPause();
        }
